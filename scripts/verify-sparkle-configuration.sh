...
require_plist_value "$INFO_PLIST" "SUEnableAutomaticChecks" "true"
require_plist_value "$INFO_PLIST" "SUEnableInstallerLauncherService" "true"
require_plist_value "$INFO_PLIST" "SUEnableDownloaderService" "true"
require_plist_value "$ENTITLEMENTS_PLIST" "com.apple.security.network.client" "false"
...
if [[ "$coreaudio_count" -lt 1 ]]; then
    add_violation "The CoreAudio Mach lookup exception is missing."
fi

if [[ "$installer_count" -lt 1 ]]; then
    add_violation "The Sparkle Installer Launcher Mach lookup exception is missing."
fi

if [[ "$downloader_count" -lt 1 ]]; then
    add_violation "The Sparkle Downloader Mach lookup exception is missing."
fi
...
echo "| Field | Status |"
echo "|---|---|"
echo "| V1.0.1 version and build | \`$(plist_value "$INFO_PLIST" "CFBundleShortVersionString") ($(plist_value "$INFO_PLIST" "CFBundleVersion"))\` |"
echo "| Fixed HTTPS feed | \`$([[ "$(plist_value "$INFO_PLIST" "SUFeedURL")" == "$EXPECTED_FEED_URL" ]] && echo configured || echo missing-or-invalid)\` |"
echo "| Automatic checks | \`$(plist_value "$INFO_PLIST" "SUEnableAutomaticChecks")\` |"
echo "| Installer Launcher service | \`$(plist_value "$INFO_PLIST" "SUEnableInstallerLauncherService")\` |"
echo "| Downloader service | \`$(plist_value "$INFO_PLIST" "SUEnableDownloaderService")\` |"
echo "| EdDSA public key | \`$public_key_status\` |"
echo "| Main-app network client | \`$(plist_value "$ENTITLEMENTS_PLIST" "com.apple.security.network.client")\` |"
