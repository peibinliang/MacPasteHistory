...
9. Run \`scripts/release-readiness-report.sh --manual-record "$manual_record_path"\` as the final release gate; add \`--allow-adhoc\` only for internal QA before distribution signing exists.
