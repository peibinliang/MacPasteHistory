...
- 采集链路会过滤常见密码、令牌、身份信息和银行卡等敏感文本模式
- 应用不会在日志中记录完整的剪贴板内容
...
剪贴板历史、图片和设置默认仅存储在本机，不会上传到云端。当前版本不会加密本地历史数据库或应用管理的图片文件；如需设备级保护，请启用 macOS 账户安全措施和 FileVault。详细说明参见[隐私政策](docs/privacy-policy.md)和[用户指南](docs/user-guide.md)。
