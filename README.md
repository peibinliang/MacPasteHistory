...
粘易是一款本地优先的 macOS 剪贴板历史工具。它常驻菜单栏，以轻量浮层呈现文本与图片历史；可选择手动粘贴、按需自动粘贴，也可主动调用 DeepSeek 润色或翻译文本。
...
- 历史、图片和设置保存在当前 Mac；只有用户主动选择 AI 润色或翻译的文本会发送给 DeepSeek
...
- 用户开启自动粘贴且授予辅助功能权限后，才会返回原应用并发送 `Command-V`；macOS 没有对所有第三方应用通用且免授权的等价公开 API
...
- 提供 AI 翻译动作，可选择简体中文、繁体中文、英语、日语或韩语作为目标语言
- 首次发送前说明文本将交由 DeepSeek 远程处理；拒绝不会影响任何本地剪贴板功能
- API Key 以仅当前用户可读写的本地明文配置文件存储，重启后不访问钥匙串；结果可继续编辑、复制、保存或粘贴
...
剪贴板历史、图片和设置默认仅存储在本机。更新检查会请求 GitHub 托管的 appcast、发布说明和更新包，但不会包含剪贴板历史。AI 润色和翻译是明确触发的远程功能，会在首次使用前提示，并仅发送当前所选文本给 DeepSeek。API Key、本地历史数据库和应用管理的图片文件没有应用级加密；如需设备级保护，请启用 macOS 账户安全措施和 FileVault。详细说明参见[隐私政策](docs/privacy-policy.md)和[用户指南](docs/user-guide.md)。
