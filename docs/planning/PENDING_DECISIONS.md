...
| PD-011 | 已决策 | V1.0.5 | V1.0.5 是否继续在没有 Developer ID Application 与 Apple 公证的情况下发布？ | 2026-08-12 产品负责人在已决定暂不配置 Developer ID 的连续发布上下文中，明确要求合并 V1.0.5 并创建新版本；本版本沿用 ad-hoc 签名发布豁免。正式打包工具的 Developer ID、公证与 `--strict-final` 门禁保持原样，不把本次豁免伪装为门禁通过。 | V1.0.5 DMG 与更新 ZIP 未公证，Gatekeeper 及辅助功能授权连续性不受保证；更新后可能再次要求辅助功能权限。本豁免不自动延伸到 V1.0.6。 |
