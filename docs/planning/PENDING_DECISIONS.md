...
| PD-009 | 待决策 | V1.0.4 | 由谁提供并维护同一 Apple Developer Team 的 Developer ID Application 证书/私钥、Team ID 与 notarization 凭据？ | 凭据只进入操作者钥匙串或 CI secret store，不提交仓库。当前机器 `security find-identity -p codesigning` 返回 0 个有效身份；V1.0.0 与 V1.0.3 均为 ad-hoc，designated requirement 分别绑定不同 CDHash，已证实会导致更新后 TCC 辅助功能权限失效。 | 不阻塞本地身份校验工具和文档实现；阻塞正式 V1.0.4 构建、真实权限连续性 QA、合并及发布。首次从 ad-hoc 迁移到 Developer ID 时仍可能需要最后一次重新授权。 |
