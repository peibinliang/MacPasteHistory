...
| PD-007 | 待决策 | V1.0.3 | 谁补验 `SystemUIServer` 菜单栏状态项与真实全局快捷键，或是否批准这两项 GUI 证据豁免？ | 2026-08-12 已在隔离 App Support 与独立 QA UserDefaults suite 中验证 Release 文本/图片列表、快速搜索、文本与图片恢复、内容动作与另存、Finder 真实图片捕获、Blocked Apps 及快速切换竞态、Automatic Paste 缺少 Accessibility 时的手动粘贴回退、三页设置读取、偏好隔离和多次重启持久化；数据库计数确认回退只记 reuse、不误记 paste。获得当次删除确认后，Settings → Storage and Data → Clear All Data 也已通过真实 GUI 执行，清空后历史/capture event 均为 0，图片与缩略图目录为空。启动对账和其余破坏性清理由自动测试覆盖。Computer Use 对 `SystemUIServer` 的两次只读访问均超时，且其按键 API 明确只能发送给指定应用、不能触发全局快捷键，因此最后两项没有伪造通过。 | Task 10.2 与合并回 `main` 保持未完成；其余 GUI 场景不再需要重复。需要人工补验两项，或由产品负责人明确批准 V1.0.3 豁免。 |
