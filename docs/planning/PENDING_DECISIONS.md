...
| PD-007 | 待决策 | V1.0.3 | 由谁在真实 macOS 应用环境执行文本/图片捕获、Blocked Apps、快速搜索、恢复/自动粘贴、内容动作、存储对账、设置、快捷键和重启的人工 smoke？ | 2026-08-12 解锁后，已构建并启动隔离数据目录的 V1.0.3 Release App，通过 VS Code 菜单执行合成文本复制，并在隔离数据库确认文本捕获；测试实例随后停止，环境变量和隔离 QA 目录已清理。Computer Use 无法读取/点击仅存在于 `SystemUIServer` 的菜单栏状态项，且其按键 API 不能触发全局快捷键，因此未伪造图片、Blocked Apps、搜索、恢复/自动粘贴、动作、设置、快捷键和重启结果。完整自动测试 372/372 通过，但不代替这些 GUI 证据。 | Task 10.2 与合并回 `main` 保持未完成。后续需要人工先点击粘易菜单栏图标并打开历史窗口后交回自动化继续，或由产品负责人明确批准本版本豁免。 |
