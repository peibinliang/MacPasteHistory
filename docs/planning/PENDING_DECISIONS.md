...
| PD-007 | 待决策 | V1.0.3 | 由谁在真实 macOS 应用环境执行文本/图片捕获、Blocked Apps、快速搜索、恢复/自动粘贴、内容动作、存储对账、设置、快捷键和重启的人工 smoke？ | 自动测试、Debug build 和本地静态门禁已通过；`docs/release/manual-qa-record.md` 的真实操作结果保持 `Not run`，不由自动化结果代填。2026-08-12 已按 `computer-use` 工作流尝试启动 GUI smoke，但 Mac 处于锁屏且自动解锁失败，未伪造操作证据。 | Task 10.2 与合并回 `main` 保持未完成，直到解锁 Mac 后完成真实 GUI 操作记录，或产品负责人明确批准本版本豁免。 |
