...
- Parameterized formal update packaging and appcast generation so new version/build pairs no longer inherit V1.0.2-specific gates.
...
- Refreshed AI token totals immediately after a newly persisted polishing response or Clear All Data, and attributed usage to the configured request model even when the provider returns a model alias.
