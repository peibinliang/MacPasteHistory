...
Settings include text/image recording toggles, launch-at-login preference, Dock icon preference, history retention days, text/image count limits, single-image size limit, and total storage cap. Cleanup runs on startup and uses these limits to bound local data growth.
