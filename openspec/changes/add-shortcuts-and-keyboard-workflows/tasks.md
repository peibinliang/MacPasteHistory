...
- [x] 4.1 Show a copy success toast after restore.
  - 实现描述：恢复成功后展示短暂 toast，例如“已复制到剪贴板”，失败时展示可理解错误。
  - 前置条件：恢复服务能返回成功或失败结果，UI 有 toast 承载位置。
  - 验收条件：恢复成功有反馈，反馈不会遮挡主要操作并会自动消失。
- [x] 4.2 Verify shortcut opening, keyboard selection, Enter restore, Escape close, and conflict handling.
