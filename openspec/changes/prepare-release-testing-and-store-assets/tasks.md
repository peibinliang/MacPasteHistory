...
- [ ] 4.4 Verify final release checklist is complete.
  ...
  - 当前进展：新增 `docs/release/manual-qa-record.md` 作为人工发布 QA 证据模板，用于记录真实设备、真实应用复制/恢复、菜单栏、双击历史项直接粘贴、Clear All Data、Launch at login 和隐私场景的 tester/date/build/evidence；新增 `scripts/release-qa-baseline.sh` 可生成当前构建、机器、Xcode、签名、Sandbox 和常见应用安装情况的 Markdown 基线，便于填入人工记录；新增 `scripts/package-release-qa-build.sh` 可生成跨机器 QA zip、SHA-256 和 manifest；新增 `scripts/verify-release-qa-package.sh` 可校验 QA 包完整性、架构、签名和 Sandbox entitlement；新增 `scripts/generate-manual-qa-fixtures.swift` 可生成无隐私人工功能 QA 样本；新增 `scripts/start-manual-release-qa-session.sh` 可一次性生成时间戳 QA 会话目录，汇总 QA 包、校验报告、基线、样本和记录模板，减少人工证据准备遗漏；新增 `scripts/prefill-manual-qa-record.sh` 并接入 QA 会话脚本，可自动预填构建、包校验、签名、当前机器和样本路径等客观字段，同时保留所有人工场景为 Not run；新增 `scripts/scan-privacy-log-safety.sh` 并将中心 Logger 默认 OSLog 隐私级别改为 private，可静态检查直接 console 输出、公开日志和明显剪贴板内容日志；新增 `scripts/validate-manual-qa-record.sh` 可在人工记录填写后检查必需章节，并在对应章节内检查 Release App Workflow 必测行、环境覆盖行、常用应用矩阵行、隐私与安全检查行、TBD/Not run 占位符、最终发布决定和签名/Team ID 明显缺口，其中必测行已包含双击粘贴场景；新增 `scripts/release-readiness-report.sh` 可汇总 Xcode 文件引用、日志隐私扫描、App Icon 素材、截图 PNG 尺寸、Release 冒烟测试、Release 安装副本预检、授权、签名身份、发布文档、人工 QA 记录和 git 状态，输出最终 Markdown readiness 报告；`--skip-release-smoke` 和 `--skip-install-preflight` 仅用于临时诊断，最终发布前仍需默认 Release smoke test 和安装副本预检通过；当前真实记录仍因未填写人工 QA 和缺少签名身份而失败，最终验收仍需这些记录实际填写、校验通过并人工复核后才能关闭。
