...
- [ ] 4.4 Verify final release checklist is complete.
  ...
  - 当前进展：新增 `docs/release/manual-qa-record.md` 作为人工发布 QA 证据模板，用于记录真实设备、真实应用复制/恢复、菜单栏、Clear All Data、Launch at login 和隐私场景的 tester/date/build/evidence；最终验收仍需这些记录实际填写并通过后才能关闭。
