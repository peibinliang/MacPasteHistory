...
- [ ] 1.1 Detect PNG data in the pasteboard.
...
- [ ] 1.5 Add support for Finder image file copies.
...
- [ ] 2.1 Create the local images directory.
...
- [ ] 4.1 Delete original and thumbnail files when image records are deleted.
  - 实现描述：扩展删除流程，在删除数据库记录时同步删除原图和缩略图文件。
  - 前置条件：图片记录包含可解析文件路径，删除 repository 已存在。
  - 验收条件：删除图片历史后数据库记录消失，原图和缩略图文件不再残留。
- [ ] 4.2 Add or wire the image recording switch.
